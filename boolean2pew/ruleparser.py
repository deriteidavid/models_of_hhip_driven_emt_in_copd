...
from boolean2pew import tokenizer, util, state
from boolean2pew.ply import yacc
...
def p_expression_binop(p):
    ...
    if p[2] == 'and'  :
        ...
        p[0] = p.parser.RULE_PLUS(p[1], p[3], p)
    elif p[2] == '-':
        p[0] = p.parser.RULE_MINUS(p[1], p[3], p)
    else:
...
class Parser(object):
	...
	#the PEW operator 
        ...
        #
        self.parser.RULE_AND = lambda a, b, p: float(a)*float(b) #min(float(a), float(b))
        self.parser.RULE_OR  = lambda a, b, p: float(a) + float(b) - float(a)*float(b)
        self.parser.RULE_PLUS = lambda a, b, p: float(a) + float(b)
